...
{
  URL this;
  this = calloc(sizeof(struct URL_T), 1);
  this->ID = 0;
...
__url_set_absolute(URL this, char *url)
{
  ...
  if (!strncasecmp(url, "ajp:", 4)){
    n = 6;
    strncpy(scheme, "ajp", 3);
  }
  if (strlen(scheme) < 3) {
    // A scheme wasn't supplied; we'll use ajp by default.
    n = 6;
    strncpy(scheme, "ajp", 3);
  }
